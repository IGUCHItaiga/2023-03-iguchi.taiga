...
int clefia(const unsigned char pt[16])
{
  ...
  int r;
  unsigned char clefia128_enc[16], clefia128_dec[16];
  unsigned char clefia192_enc[16], clefia192_dec[16];
  unsigned char clefia256_enc[16], clefia256_dec[16];
  ...
  ClefiaEncrypt(clefia128_enc, pt, rk, r);
  printf("ciphertext: "); BytePut(clefia128_enc, 16);
  //clefia128_enc = dst;
  /* decryption */
  ByteCpy(ct, clefia128_enc, 16);
  r = ClefiaKeySet(rk, skey, 128);
  ClefiaDecrypt(clefia128_dec, ct, rk, r);
  printf("plaintext : "); BytePut(clefia128_dec, 16);

  /* for 192-bit key */
  printf("--- CLEFIA-192 ---\n");
  /* encryption */
  r = ClefiaKeySet(rk, skey, 192);
  ClefiaEncrypt(clefia192_enc, pt, rk, r);
  printf("ciphertext: "); BytePut(clefia192_enc, 16);
  /* decryption */
  ByteCpy(ct, clefia192_enc, 16);
  r = ClefiaKeySet(rk, skey, 192);
  ClefiaDecrypt(clefia192_dec, ct, rk, r);
  printf("plaintext : "); BytePut(clefia192_dec, 16);

  /* for 256-bit key */
  printf("--- CLEFIA-256 ---\n");
  /* encryption */
  r = ClefiaKeySet(rk, skey, 256);
  ClefiaEncrypt(clefia256_enc, pt, rk, r);
  printf("ciphertext: "); BytePut(clefia256_enc, 16);
  /* decryption */
  ByteCpy(ct, clefia256_enc, 16);
  r = ClefiaKeySet(rk, skey, 256);
  ClefiaDecrypt(clefia256_dec, ct, rk, r);
  printf("plaintext : "); BytePut(clefia256_dec, 16);
