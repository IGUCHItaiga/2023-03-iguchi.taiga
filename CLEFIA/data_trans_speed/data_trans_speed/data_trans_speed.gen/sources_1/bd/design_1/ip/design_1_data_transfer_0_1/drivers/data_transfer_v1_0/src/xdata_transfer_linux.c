...
typedef struct {
    ...
    XData_transfer_uio_map maps[ MAX_UIO_MAPS ];
} XData_transfer_uio_info;

/***************** Variable Definitions **************************************/
static XData_transfer_uio_info uio_info;
...
static int uio_info_read_name(XData_transfer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XData_transfer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XData_transfer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XData_transfer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XData_transfer_Initialize(XData_transfer *InstancePtr, const char* InstanceName) {
	XData_transfer_uio_info *InfoPtr = &uio_info;
