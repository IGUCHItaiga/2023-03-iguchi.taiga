...
extern "C" void data_transfer(char*, char*, char*);
extern "C" void apatb_data_transfer_hw(volatile void * __xlx_apatb_param_pt, volatile void * __xlx_apatb_param_skey, volatile void * __xlx_apatb_param_ct) {
  ...
  // Collect __xlx_skey__tmp_vec
  vector<sc_bv<8> >__xlx_skey__tmp_vec;
  for (int j = 0, e = 32; j != e; ++j) {
    __xlx_skey__tmp_vec.push_back(((char*)__xlx_apatb_param_skey)[j]);
  }
  int __xlx_size_param_skey = 32;
  int __xlx_offset_param_skey = 0;
  int __xlx_offset_byte_param_skey = 0*1;
  char* __xlx_skey__input_buffer= new char[__xlx_skey__tmp_vec.size()];
  for (int i = 0; i < __xlx_skey__tmp_vec.size(); ++i) {
    __xlx_skey__input_buffer[i] = __xlx_skey__tmp_vec[i].range(7, 0).to_uint64();
  }
  ...
  // DUT call
  data_transfer(__xlx_pt__input_buffer, __xlx_skey__input_buffer, __xlx_ct__input_buffer);
...
// print __xlx_apatb_param_skey
  sc_bv<8>*__xlx_skey_output_buffer = new sc_bv<8>[__xlx_size_param_skey];
  for (int i = 0; i < __xlx_size_param_skey; ++i) {
    __xlx_skey_output_buffer[i] = __xlx_skey__input_buffer[i+__xlx_offset_param_skey];
  }
  for (int i = 0; i < __xlx_size_param_skey; ++i) {
    ((char*)__xlx_apatb_param_skey)[i] = __xlx_skey_output_buffer[i].to_uint();
  }
// print __xlx_apatb_param_ct
  ...
  for (int i = 0; i < __xlx_size_param_ct; ++i) {
    ((char*)__xlx_apatb_param_ct)[i] = __xlx_ct_output_buffer[i].to_uint();
  }
}
