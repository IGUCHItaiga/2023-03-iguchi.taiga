...
typedef struct {
    ...
    XClefia_dec_uio_map maps[ MAX_UIO_MAPS ];
} XClefia_dec_uio_info;

/***************** Variable Definitions **************************************/
static XClefia_dec_uio_info uio_info;
...
static int uio_info_read_name(XClefia_dec_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XClefia_dec_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XClefia_dec_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XClefia_dec_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XClefia_dec_Initialize(XClefia_dec *InstancePtr, const char* InstanceName) {
	XClefia_dec_uio_info *InfoPtr = &uio_info;
