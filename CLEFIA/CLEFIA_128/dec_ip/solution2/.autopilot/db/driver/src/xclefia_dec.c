...
u32 XClefia_dec_Get_skey_BaseAddress(XClefia_dec *InstancePtr) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    return (InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE);
}

u32 XClefia_dec_Get_skey_HighAddress(XClefia_dec *InstancePtr) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    return (InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_HIGH);
}

u32 XClefia_dec_Get_skey_TotalBytes(XClefia_dec *InstancePtr) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    return (XCLEFIA_DEC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + 1);
}

u32 XClefia_dec_Get_skey_BitWidth(XClefia_dec *InstancePtr) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    return XCLEFIA_DEC_CONTROL_WIDTH_SKEY;
}

u32 XClefia_dec_Get_skey_Depth(XClefia_dec *InstancePtr) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    return XCLEFIA_DEC_CONTROL_DEPTH_SKEY;
}

u32 XClefia_dec_Write_skey_Words(XClefia_dec *InstancePtr, int offset, word_type *data, int length) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr -> IsReady == XIL_COMPONENT_IS_READY);

    int i;

    if ((offset + length)*4 > (XCLEFIA_DEC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}

u32 XClefia_dec_Read_skey_Words(XClefia_dec *InstancePtr, int offset, word_type *data, int length) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr -> IsReady == XIL_COMPONENT_IS_READY);
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + (offset + i)*4);
    }
    return length;
}

u32 XClefia_dec_Write_skey_Bytes(XClefia_dec *InstancePtr, int offset, char *data, int length) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr -> IsReady == XIL_COMPONENT_IS_READY);

    int i;

    if ((offset + length) > (XCLEFIA_DEC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + offset + i) = *(data + i);
    }
    return length;
}

u32 XClefia_dec_Read_skey_Bytes(XClefia_dec *InstancePtr, int offset, char *data, int length) {
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr -> IsReady == XIL_COMPONENT_IS_READY);
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XCLEFIA_DEC_CONTROL_ADDR_SKEY_BASE + offset + i);
    }
    return length;
}
