...
u32 XClefia_enc_Get_skey_BaseAddress(XClefia_enc *InstancePtr) {
    ...
    return (InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE);
}
...
u32 XClefia_enc_Get_skey_HighAddress(XClefia_enc *InstancePtr) {
    ...
    return (InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_HIGH);
}
...
u32 XClefia_enc_Get_skey_TotalBytes(XClefia_enc *InstancePtr) {
    ...
    return (XCLEFIA_ENC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + 1);
}
...
u32 XClefia_enc_Get_skey_BitWidth(XClefia_enc *InstancePtr) {
    ...
    return XCLEFIA_ENC_CONTROL_WIDTH_SKEY;
}
...
u32 XClefia_enc_Get_skey_Depth(XClefia_enc *InstancePtr) {
    ...
    return XCLEFIA_ENC_CONTROL_DEPTH_SKEY;
}
...
u32 XClefia_enc_Write_skey_Words(XClefia_enc *InstancePtr, int offset, word_type *data, int length) {
    ...
    if ((offset + length)*4 > (XCLEFIA_ENC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(int *)(InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + (offset + i)*4) = *(data + i);
    }
    return length;
}
...
u32 XClefia_enc_Read_skey_Words(XClefia_enc *InstancePtr, int offset, word_type *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int *)(InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + (offset + i)*4);
    }
    return length;
}
...
u32 XClefia_enc_Write_skey_Bytes(XClefia_enc *InstancePtr, int offset, char *data, int length) {
    ...
    if ((offset + length) > (XCLEFIA_ENC_CONTROL_ADDR_SKEY_HIGH - XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(char *)(InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + offset + i) = *(data + i);
    }
    return length;
}
...
u32 XClefia_enc_Read_skey_Bytes(XClefia_enc *InstancePtr, int offset, char *data, int length) {
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(char *)(InstancePtr->Control_BaseAddress + XCLEFIA_ENC_CONTROL_ADDR_SKEY_BASE + offset + i);
    }
    return length;
}
